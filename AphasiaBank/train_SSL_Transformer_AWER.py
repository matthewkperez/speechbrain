...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # _ = self.modules.seq_lin(pred)
        # _ = self.hparams.log_softmax(pred)
        # print(f"Trans pred: {pred.shape}")
        # Augmented with paraphasia layer for seq2seq log-probabilities
        pred = self.modules.para_seq_lin(pred)
        # pred = self.modules.seq_lin(pred)
        # print(f"lin pred: {pred.shape}")
        # exit()
        ...
        return p_seq, wav_lens, hyps, dec_mha

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss (CTC+NLL) given predictions and targets."""

        # (p_ctc, p_seq, wav_lens, hyps,dec_attn) = predictions
        (p_seq, wav_lens, hyps,dec_attn) = predictions
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if current_epoch % valid_search_interval == 0 or current_epoch==1 or (
                ...
                target_words = [wrd.split(" ") for wrd in batch.filter_control] # AB
                self.wer_metric.append(ids, predicted_words, target_words)
                self.cer_metric.append(ids, predicted_words, target_words)
                # print(f"T: {target_words}")
                # print(f"P: {predicted_words}")
    ...
    def on_fit_start(self):
        ...
        if self.checkpointer is not None:
            ...
            if self.distributed_launch:
                # change Transformer
                if self.modules.Transformer.module.custom_tgt_module.layers[0].emb.Embedding.weight.shape[0] != self.hparams.para_output_neurons:
                    print("fixing transformer size")
                    para_index = self.tokenizer.piece_to_id('!')
                    new_emb = torch.nn.Embedding(1,self.hparams.d_model).to(self.device)

                    # concat    
                    weights_before_index = self.modules.Transformer.module.custom_tgt_module.layers[0].emb.Embedding.weight[:para_index, :]  # Extract rows 0 to 3
                    weights_after_index = self.modules.Transformer.module.custom_tgt_module.layers[0].emb.Embedding.weight[para_index:, :]  # Extract rows 4 to end

                    # Now, concatenate these three parts: before index, new weights, and after index
                    new_weights = torch.cat([weights_before_index, new_emb.weight, weights_after_index], 0)
                    # Add new weights to transformer embedding
                    updated_emb = torch.nn.Embedding(hparams['para_output_neurons'],hparams['d_model'])
                    updated_emb.weight = torch.nn.Parameter(new_weights)
                    self.modules.Transformer.module.custom_tgt_module.layers[0].emb.Embedding = updated_emb
                
                    
                    print("update para_seq_lin")
                    # self.modules.para_seq_lin.module.w.weight = torch.nn.Parameter(self.modules.para_seq_lin.module.w.weight.clone())
                    # self.modules.para_seq_lin.module.w.bias = torch.nn.Parameter(self.modules.para_seq_lin.module.w.bias.clone())
                    # update para_seq_lin
                    with torch.no_grad():
                        # print(f"seq: {self.modules.seq_lin.module.w.weight[:,0]}")
                        # print(f"w: {self.modules.para_seq_lin.module.w.weight[:,0]}")
                        self.modules.para_seq_lin.module.w.weight[:para_index, :] = self.modules.seq_lin.module.w.weight[:para_index, :]
                        self.modules.para_seq_lin.module.w.weight[para_index+1:, :] = self.modules.seq_lin.module.w.weight[para_index:, :]

                        self.modules.para_seq_lin.module.w.bias[:para_index] = self.modules.seq_lin.module.w.bias[:para_index]  # Extract rows 0 to 3
                        self.modules.para_seq_lin.module.w.bias[para_index+1:] = self.modules.seq_lin.module.w.bias[para_index:]  # Extract rows 4 to end
                        # print(f"w2: {self.modules.para_seq_lin.module.w.weight[:,0]}")
                        # exit()
...
def dataio_prepare(hparams,tokenizer):
    ...
    @sb.utils.data_pipeline.provides(
        "wrd", "filter_control", "tokens_list", "tokens_bos", "tokens_eos", "tokens"
    )
    def text_pipeline(wrd,aug_para):
        yield wrd
        # print(f"aug_para: {aug_para}")
        filter_control = " ".join([a[:-2] if a.endswith("C") else a[:-2]+'!' for a in aug_para.split()])
        yield filter_control
        # print(f"filter_control: {filter_control}")
        # if '!' in filter_control:
        #     exit()
        tokens_list = tokenizer.sp.encode_as_ids(filter_control)
        yield tokens_list
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "sig", "wrd", "filter_control", "tokens_bos", "tokens_eos", "tokens"],
    )
...
if __name__ == "__main__":
    ...
    tokenizer = SentencePiece(
        ...
        vocab_size=hparams["para_output_neurons"],
    ...
    if "use_language_modelling" in hparams:
        if hparams["use_language_modelling"]:
            ...
            labels  = [asr_brain.tokenizer.id_to_piece(id) for id in range(asr_brain.tokenizer.get_piece_size())]
