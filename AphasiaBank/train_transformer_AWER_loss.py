...
def create_disfluency_mask(targets, disf_token_ids):
    # Create a mask that is True for all disfluency tokens in the target sequence and False otherwise
    mask = torch.zeros_like(targets, dtype=torch.bool)
    for disf_token_id in disf_token_ids:
        mask |= (targets == disf_token_id)
    return mask
...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        enc_out, pred, dec_attn = self.modules.Transformer(
        ...
        if stage == sb.Stage.TRAIN:
            ...
            # print(f"test search: {self.hparams.test_search}")

        return p_ctc, p_seq, wav_lens, hyps, dec_attn
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # # Compute weighted cross-entropy loss for /C and /P token
        # paraphasia_toks = [self.tokenizer.piece_to_id(x) for x in ['/', 'C','P']]
        # # paraphasia_toks = [self.tokenizer.lab2ind[x] for x in ['/', 'C','P']]
        # para_weight = 1
        # weights = [para_weight if i in paraphasia_toks else 1 for i in range(len(self.tokenizer.lab2ind.keys()))]
        # weights = torch.tensor(weights, dtype=torch.float32).to(self.device)
        # loss_seq = self.hparams.seq_cost(
        #     p_seq, tokens_eos, length=tokens_eos_lens, weight=weights
        # ).sum()
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if current_epoch % valid_search_interval == 0 or current_epoch == 1 or (
                ...
                target_words = [wrd.split(" ") for wrd in batch.aug_para] # AB
...
def dataio_prepare(hparams,tokenizer):
    ...
    def text_pipeline(wrd,aug_para):
        yield wrd
        tokens_list = tokenizer.sp.encode_as_ids(aug_para)
        yield tokens_list
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "sig", "wrd", "tokens_bos", "tokens_eos", "tokens", "aug_para"],
    )
