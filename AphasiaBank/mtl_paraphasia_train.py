...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        char_emb = self.modules.enc(feats)
    

        # Compute outputs
        p_tokens = None
        ctc_logits = self.modules.ctc_lin(char_emb)
        #probs
        ...
        # print(f"p_ctc: {p_ctc.shape}")
        # frame_tokens = torch.argmax(p_ctc, dim=-1)
        # # split based on ' ' token
        # print(f"frame_tokens: {frame_tokens.shape}")
        # batch_list = []
        # for b_idx, s in enumerate(frame_tokens):
        #     print(f"s: {s}")
        #     # get start and end indices for a given word
        #     tup_list = []
        #     start=0
        ...
        #             # end of word
        #             if start != idx:
        #                 # only add if not consecutive spaces
        #                 tup_list.append((start,idx))
        #             start = idx+1
        ...
        #     # split w2v emb into word embs
        #     # print(f"tup_list: {tup_list}")
        #     w_embs = []
        #     for w_tup in tup_list:
        #         b_emb = feats[b_idx,w_tup[0]:w_tup[1],:]
        #         w_pool = torch.mean(b_emb,dim=0)
        #         w_embs.append(w_pool)
        #     w_embs = torch.vstack(w_embs)
        #     # print(f"w_embs: {w_embs.shape}")
        #     batch_list.append(w_embs)
        
        # word_rep = torch.vstack(batch_list)
        # # print(f"word_rep: {word_rep.shape}")
        # print(f"batch.wrd: {batch.wrd}")
        # exit()
            
    
        para_emb = self.modules.para_enc(feats)
        para_logits = self.modules.para_lin(para_emb)
        p_para = self.hparams.log_softmax(para_logits)

        return p_ctc, p_para, wav_lens, p_tokens

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss (CTC+NLL) given predictions and targets."""

        p_ctc, p_para, wav_lens, predicted_tokens = predictions

        ids = batch.id
        tokens, tokens_lens = batch.tokens
        gt_para = batch.paraphasia
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.provides(
        "wrd", "char_list", "tokens_list", "tokens", "paraphasia"
    )
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "sig", "wrd", "char_list", "tokens", "paraphasia"],
    )
...
if __name__ == "__main__":
    ...
    # print(f"w2v model: {asr_brain.modules.wav2vec2}")
    # exit()
