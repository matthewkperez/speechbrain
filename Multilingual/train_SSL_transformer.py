...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            # print(f"test_search lm: {self.hparams.test_search.lm_weight}")
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # cleanup
        del p_ctc, p_seq, wav_lens, hyps,dec_attn
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        elif stage == sb.Stage.TEST and sb.utils.distributed.if_main_process():
            self.hparams.train_logger.log_stats(
...
def dataio_prepare(hparams,tokenizer):
    ...
    # # Convert lang_id to num
    # LANG_ID_NUM = {'en':0, 'fr':1, 'it':2, 'sp':3}
    # train_data.data = {utt_id:{k: (LANG_ID_NUM[v] if k == 'lang_id' else v) for k,v in data_dict.items()} for utt_id,data_dict in train_data.data.items()}
    # lang_filter_num = LANG_ID_NUM[hparams["lang_id"]]

    if hparams["sorting"] == "ascending":
        # we sort training data to speed up training and get better results.
        train_data = train_data.filtered_sorted(sort_key="duration",
            key_max_value={"duration": hparams["max_length"]},
            # key_max_value={"duration": 1, "lang_id": lang_filter_num},
    ...
    # valid_data.data = {utt_id:{k: (LANG_ID_NUM[v] if k == 'lang_id' else v) for k,v in data_dict.items()} for utt_id,data_dict in valid_data.data.items()}
    ...
    # test_data.data = {utt_id:{k: (LANG_ID_NUM[v] if k == 'lang_id' else v) for k,v in data_dict.items()} for utt_id,data_dict in test_data.data.items()}
    ...
    @sb.utils.data_pipeline.takes("wrd","lang_id")
    @sb.utils.data_pipeline.provides(
        "wrd", "tokens_list", "tokens_bos", "tokens_eos", "tokens"
    )
    def text_pipeline(wrd,lang_id):
        yield wrd
        # print(f"word: {wrd}")
        # char_list = list(wrd)
        ...
        # replace bos with lang_id
        if hparams['lang_id'] == 'multi':
            lang_id_str = f'[{lang_id}]'
            lang_id_tok = tokenizer.sp.piece_to_id(lang_id_str)
            tokens_bos = torch.LongTensor([lang_id_tok] + (tokens_list))
        else:
...
if __name__ == "__main__":
    ...
    if hparams['lang_id'] == 'multi':
        tokenizer = SentencePiece(
            ...
            user_defined_symbols="[en],[fr],[it],[sp]"
        )
    ...
    assert len(tokens.keys()) == hparams['output_neurons'], f"Output neurons ({hparams['output_neurons']}) do not match tokens ({len(tokens.keys())})"
    # exit()
    ...
    #         asr_brain.evaluate(
    #             valid_data, stage=sb.Stage.TEST, test_loader_kwargs=hparams["valid_dataloader_opts"]
    #         )
