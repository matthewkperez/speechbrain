...
def check_inf_nan(tensor, name):
    if torch.isinf(tensor).any():
        print(f'Infinity found in {name}')
...
class ASR(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # print(f"p_ctc: {p_ctc.shape} | {torch.argmax(p_ctc, dim=-1)}")
...
if __name__ == "__main__":
    ...
    assert len(asr_brain.tokenizer.lab2ind.keys()) == hparams['output_neurons'], f"tokenizer ({len(asr_brain.tokenizer.lab2ind.keys())}) does not match output_neurons ({hparams['output_neurons']})"
