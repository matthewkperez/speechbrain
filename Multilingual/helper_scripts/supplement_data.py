            ...
            for i,row in df.iterrows():
                if row['ID'] not in utt2wrd_label or len(row['wrd'].split()) != len(utt2wrd_label[row['ID']]):
                    ...
                    if para_type=='multi':
                        ...
                        para_bool.append(max([0 if para == 'c' else 1 for word,para in zip(row['wrd'].split(), utt2wrd_label[row['ID']])]))
                    elif para_type == 'pn':
                        aug_para_wrds.append(" ".join([f"{word}/C" if para == 'c' else f"{word}/P" for word,para in zip(row['wrd'].split(), utt2wrd_label[row['ID']])]))
                        para_bool.append(max([1 if para in ['p','n'] else 0 for word,para in zip(row['wrd'].split(), utt2wrd_label[row['ID']])]))
                    elif para_type == 'p':
                        aug_para_wrds.append(" ".join([f"{word}/P" if para == 'p' else f"{word}/C" for word,para in zip(row['wrd'].split(), utt2wrd_label[row['ID']])]))
                        para_bool.append(max([1 if para=='p' else 0 for word,para in zip(row['wrd'].split(), utt2wrd_label[row['ID']])]))
                    elif para_type == 'n':
                        aug_para_wrds.append(" ".join([f"{word}/P" if para == 'n' else f"{word}/C" for word,para in zip(row['wrd'].split(), utt2wrd_label[row['ID']])]))
                        para_bool.append(max([1 if para=='n' else 0 for word,para in zip(row['wrd'].split(), utt2wrd_label[row['ID']])]))
